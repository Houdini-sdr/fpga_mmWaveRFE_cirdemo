module CIR_Extract_1x1_Tx_top #(
    ...
    localparam FRAME_WORDS = ((17+9)*128 + payload_len*2) / 8;
...
//        assign chips_out[256-1-32*i -: 32] = sector_id_ex[i] ? Ga32 : ~Ga32;
    ...
    // The RFDC DAC stream must keep accepting valid samples. Drive zeros
    // during packet gaps/done instead of dropping TVALID and starving the DAC.
    assign I_tvalid = rst_n;
    assign Q_tvalid = rst_n;
        ...
        if (rst) begin
            ...
            if (!en) begin  // Kai 05/28/2024
                ...
                if (!Tx_done) begin
                    ...
                    if (cnt < FRAME_WORDS) begin // Kai 04/21/2025
                        o_valid <= 1;
                    ...
                    else if (cnt < FRAME_WORDS + DELAY) begin // Kai 04/21/2025
                        o_valid <= 0;
                        cnt <= cnt + 1;
                    end
                    else begin 
